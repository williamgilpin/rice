...
if dataset == "mccalla":
    ...
    for item, condition in McCalla():
        ...
        run_benchmark_model(item, output_fname, nval=100, DREAM4_flag=False, models=models, n_datasets=args.nsplit)
    ...
    for item, condition in Kuramoto():
        ...
        run_benchmark_model(item, output_fname, nval=nval, DREAM4_flag=False, save_matrix=True, models=models, n_datasets=args.nsplit)
    ...
    for item, (engine,) in Smoketest():
        ...
        run_benchmark_model(item, output_fname, nval=nval, DREAM4_flag=False, models=models, n_datasets=args.nsplit)

elif dataset == "dream4":
    from dataloaders import DREAM4
    for item, (nval,) in DREAM4():
        output_fname = f"DREAM4_InSilico{nval}_scores.txt"
        run_benchmark_model(item, output_fname, nval=nval, DREAM4_flag=True, models=models, n_datasets=args.nsplit)
