...
# from hccm.hccm.ccm import CausalDetection, filter_loops

sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
from models import WeightedCorrelations, LaggedCorrelations
...
output_fname = f"{ORGANISM_NAME}{higher_order_str}_scores_wcorr.txt"
...
for data_fname in data_fnames[:25]:
    ...
    # model = WeightedCorrelations(method="spearman", tau=0)
    # cmat = model.fit(X)
    ...
    # scores = compute_metrics(amat, cmat.T, verbose=False, check_transpose=False)
    # print(scores, flush=True)
    # break

    results[seed_val] = scores
    results.transpose().to_csv(output_fname, sep="\t")
